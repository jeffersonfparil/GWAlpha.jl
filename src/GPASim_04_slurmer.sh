...
# EXAMPLE 1:
# ./GPASim_04_slurmer.sh /data/cephfs/punim0543/jparil 10 0.001 0.25 -0.25 1 32
# EXAMPLE 2:
...
#             ./GPASim_04_slurmer.sh /data/cephfs/punim0543/jparil ${nqtl} ${mr} ${fgs} ${bgs} ${grad} ${rep} 32
#            done
#         done
#       done
#     done
...
nCores=$(echo "${VAR_mem} / ${VAR_reqmem}" | bc) # the number of cores for parallelizing GPAS cross-validation is restricted by the total memory available

# Add module loading
head -n1 \${GEN_PRED_SRC_DIR}/GPASim_04_GWAS_GP.sh > temp_LOLSIM_${VAR_rep}rep_${VAR_nQTL}QTL_${VAR_migration}mr_${VAR_foreground_selection}fgs_${VAR_background_selection}bgs_${VAR_gradient}grad.sh
...
tail -n+2 \${GEN_PRED_SRC_DIR}/GPASim_04_GWAS_GP.sh >> temp_LOLSIM_${VAR_rep}rep_${VAR_nQTL}QTL_${VAR_migration}mr_${VAR_foreground_selection}fgs_${VAR_background_selection}bgs_${VAR_gradient}grad.sh
