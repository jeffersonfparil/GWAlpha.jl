...
# filename_phen_py = "test_1kloci_g1000_p01_POOLS_PHENO.py" 	# for GWAlpha
# filename_phen_csv = "test_1kloci_g1000_p01_POOLS_PHENO.csv" # for GWAlpha_RR
# MAF = 1.0/(2*5*55)
# COVARIATE_NPSTAT = DelimitedFiles.readdlm("test_1kloci_g1000_p01_POOLS_GENO_COVARIATE_NPSTAT.csv", ',')
# COVARIATE_FST = DelimitedFiles.readdlm("test_1kloci_g1000_p01_POOLS_GENO_COVARIATE_FST.csv", ',')
# ##########################################
...
function PoolGPAS(filename_sync::String, filename_phen::String; MAF::Float64=0.01, DEPTH::Int64=10, MODEL::String="FIXED_GWAlpha", COVARIATE::Any=nothing, FPR::Float64=0.01, PARALLEL::Bool=false)
	...
	if MODEL == "FIXED_GWAlpha"
		if (PARALLEL == false)
			...
			OUT = GWAlpha_ML_parallel(filename_sync, filename_phen, MAF)
		end
		COVAR_EFF = nothing
	else
